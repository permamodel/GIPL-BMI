...
!print*,'TAUM',TAUM,'time_p:',time_p,'temp_o',temp_o,'alf',ALF(2),'bet',BET(2)
!stop
...
22  continue
    ...
    do i_grd=2,n_grd-1
        ...
        !print*,'C_app,lambda:',fapp_hcap(temp_o,isite,i_grd),ftcon(temp_o(i_grd),isite,i_grd,time_p)
        B=2.D0*ftcon(temp_o(i_grd+1),isite,i_grd+1,time_p)/(dz(i_grd+1)*(dz(i_grd)+dz(i_grd+1)))
        !print*,'lambda:',ftcon(temp_o(i_grd+1),isite,i_grd+1,time_p)
        C=A+B+D
        ALF(i_grd+1)=B/(C-A*ALF(i_grd))
        BET(i_grd+1)=(A*BET(i_grd)+D*temps(i_grd))/(C-A*ALF(i_grd))
        !print*,D,A,B,C,ALF(i_grd+1),BET(i_grd+1)
        !if(i_grd==n_grd-1)stop
    enddo
